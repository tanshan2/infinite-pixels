...
浏览器视口能力请求 1672 × 941 后仍固定输出 1280 × 720，因此保留原始 16:9 比例做归一化对照，并记录该工具限制；响应式规则另外以 390 × 844 实测。
...
- 独立代码审查发现刷新状态、键盘焦点、按键冒泡、减少动态效果、触摸尺寸、高倍缩放标签和动画调度边界。
- 修正：刷新强制恢复休眠；Enter/Space 激活后键盘控件立即显示；地点按钮按键不再被舞台截获；减少动态效果保持原缩放；语言、地点和缩放控件均达到 44 × 44 像素命中区。
- 修正：地点文字层与画布共享高倍缩放变换；地点闪烁并入单一低频动画调度器；会话存储读写增加容错；传送门进入态补上暖金色光晕。

## 交互验证

- 双击地球可从 `globe3d-stage` 进入 `is-active`，提示改为“拖动旋转 · 滚动缩放”。
- 键盘聚焦地球后按回车键也可唤醒。
- 唤醒放大动画在开始、中间与结束三个时间点产生不同截图哈希；减少动态效果的纯函数测试确认不改变缩放。
...
- 浏览器控制台无错误或警告。
- 语言按钮、地点按钮和缩放控件的实测命中尺寸均不小于 44 × 44 像素。
- 高倍缩放达到 1.3 时，画布与地点文字层的计算变换同为 `scale(1.18182)`。
- 激活后刷新页面重新回到休眠状态。
